...
import { Database, LayoutDashboard, Workflow, Cpu, Link as LinkIcon, BarChart, Server, ShieldCheck, Code, Globe, ArrowLeft, CheckCircle2, PackageCheck, ArrowRight, Sparkles, MessageSquare } from "lucide-react";
...
export default async function ServiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Filter projects relevant to this service category
  const relatedProjects = featuredProjects.filter((p) => {
    if (id === "erp" && p.category.toLowerCase().includes("erp")) return true;
    if (id === "web-apps" && p.category.toLowerCase().includes("web")) return true;
    if (id === "automation" && p.category.toLowerCase().includes("automation")) return true;
    return true; // Default fallback matches
  }).slice(0, 3);
