...
// Curated Visual Styling Presets for Principle Cards
const cardStyles = [
  {
    icon: Compass,
...
export default function WhyMe() {
  ...
  useEffect(() => {
    fetch("/api/philosophy")
      .then((res) => res.json())
      .then((fetched) => {
        if (fetched && (fetched.headingPart1 || fetched.badge)) {
          setData((prev) => ({
            ...prev,
            ...fetched,
            principles: Array.isArray(fetched.principles) && fetched.principles.length > 0 ? fetched.principles : prev.principles,
          }));
        }
      })
  ...
  const badge = data?.badge || defaultPhilosophy.badge;
  const headingPart1 = data?.headingPart1 || defaultPhilosophy.headingPart1;
  const headingPart2 = data?.headingPart2 || defaultPhilosophy.headingPart2;
  const description = data?.description || defaultPhilosophy.description;
            ...
            return (
              ...
              >
                ...
                <div 
                  ...
                  style={{
                    background: `radial-gradient(circle at 50% 0%, ${style.bgGlow}, transparent 70%)`
                  }}
                />

                {/* Top Subtle Light Line Accent */}
                <div 
                  className={`absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r ${style.gradient} opacity-0 group-hover:opacity-100 transition-opacity duration-500`} 
                />
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mb-8">
                    ...
                    <div className="relative">
                      <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${style.gradient} p-0.5 shadow-md group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300`}>
                      ...
                      {/* Icon Ambient Glow */}
                      <div 
                        className={`absolute inset-0 rounded-2xl bg-gradient-to-br ${style.gradient} blur-lg opacity-40 group-hover:opacity-80 transition-opacity duration-300 -z-10`} 
                      />
                    </div>

                    {/* Tag & Watermark Number */}
                    <div className="flex items-center gap-3">
                      <span className={`text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border ${style.badgeColor}`}>
                        {item.tag}
                      </span>
